...
SRS.prototype.doneLine = function(line, wasPerfect) {
	...
	if ((SRSLines[lineIndex].timeDue < theTime || SRSLines[lineIndex].timeDue == this.unseenLineTime) && wasPerfect) {
		SRSLines[lineIndex].nextBox++;
		SRSLines[lineIndex].timeDue = theTime + this.timeToNextShowing[SRSLines[lineIndex].nextBox];
...
function sendLineToServer(moves, timeDue, nextBox, colour, timestried) {
	...
	$.post( "api.php", { action: 'update', username: username, password: password, moves: moves, timedue: timeDue, nextbox: nextBox, colour: colour }, function( data ) {
		if (data == 'notloggedin') {

...
function start() {
    ...
    gltf
        ...
        .then(json => updatePrimitives(gl, json))
        .then(
            function () {
...
function loadBuffers(gl, gltf) {
    ...
    for (let buffer of gltf.buffers) {
        if (buffer.reader === undefined) {
            ...
            fetch("resources/models/" + buffer.uri, {cache: "no-store"})
...
function updatePrimitives(gl, json) {
    let nodes   = json.nodes
        .filter(x => x.name.startsWith(
            document.getElementById("shipPick").value
        ));
    primitives = [];
    ...
    document.getElementById("shipPick").onchange = 
        () => updatePrimitives(gl, json);
    return primitives;
}
